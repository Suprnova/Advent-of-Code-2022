...
func main() {
	...
	for sc.Scan() {
		...
		// use a temporary stack to hold the items that are being moved
		var tempStack Stack

		for i := 0; i < count; i++ {
			// pop the item from the source stack and push it onto the temp stack
			tempStack.Push(stacks[sourceStack-1].Pop())
		}
		for i := 0; i < count; i++ {
			// pop the items from the temp stack back onto the destination stack, effectively reversing them
