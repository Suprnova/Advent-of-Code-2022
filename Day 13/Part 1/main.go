...
func main() {
	...
	for sc.Scan() {
		// parse the input, save it as a list to left and right
		left := parseInput(sc.Text())
		sc.Scan()
		right := parseInput(sc.Text())

		// check the ordered status, 2 means they're ordered
		if orderedStatus(&left, &right) == 2 {
