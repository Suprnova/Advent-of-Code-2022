...
func main() {
	...
	// draw a floor 2 spaces below the highest y value
	for i := range cave[highestY+2] {
		cave[highestY+2][i] = '#'
	}
...
func dropSand(cave *[][]rune, x, y int) {
	if (*cave)[y][x] == '.' {
		...
		if (*cave)[y+1][x] == '.' {
			...
			// if we can't move the sand and we're at the source,
			// then we've clogged the source, return
			if x == 500 && y == 0 {
				return
			}
			// otherwise if we can't move the sand, we know it's settled. drop a new piece of sand at the source
			dropSand(cave, 500, 0)
		}
	}
}
